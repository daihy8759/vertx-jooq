...
/**
 * Abstract base class to reduce duplicate code in the different VertxDAO
 * implementations.
 * 
 * @param <R>                the <code>Record</code> type.
 * @param <P>                the POJO-type
 * @param <T>                the Key-Type
 * @param <FIND_MANY>        the result type returned for all
 *                           findManyXYZ-operations. This varies on the
 *                           VertxDAO-subtypes, e.g. {@code Future<List<P>>}.
 * @param <FIND_ONE>         the result type returned for all
 *                           findOneXYZ-operations. This varies on the
 *                           VertxDAO-subtypes , e.g. {@code Future<P>}.
 * @param <EXECUTE>          the result type returned for all insert, update and
 *                           delete-operations. This varies on the
 *                           VertxDAO-subtypes, e.g. {@code Future<Integer>}.
 * @param <INSERT_RETURNING> the result type returned for the
 *                           insertReturning-operation. This varies on the
 *                           VertxDAO-subtypes, e.g. {@code Future<T>}.
 */
public abstract class AbstractVertxDAO<R extends UpdatableRecord<R>, P, T, FIND_MANY, FIND_ONE, EXECUTE, INSERT_RETURNING>
    ...
    public EXECUTE update(P object) {
        ...
        return queryExecutor().execute(dslContext -> {
            ...
            Map<String, Object> valuesToUpdate = Arrays.stream(record.fields()).collect(HashMap::new,
                    (m, f) -> m.put(f.getName(), f.getValue(record)), HashMap::putAll);
            return dslContext.update(getTable()).set(valuesToUpdate).where(where);
        });
    }
    ...
    public INSERT_RETURNING insertReturningPrimary(P object) {
        ...
        return queryExecutor().insertReturning(dslContext -> dslContext.insertInto(getTable())
                .set(newRecord(dslContext, object)).returning(key.getFields()), keyConverter());
    }
